# Hypersonic Defense Screener — Ticker Universe

**Version:** 0.3  
**Date:** June 2026  
**Status:** Active  

---

## Overview

This document defines the investable universe for the 
Hypersonic Defense Screener. Inclusion requires a company 
to meet ALL of the following criteria:

1. Publicly traded on a US exchange (NYSE or NASDAQ)
2. Meaningful revenue exposure to next-generation defense 
   technology or critical supply chain components thereof
3. Headquartered in the United States
4. Sufficient liquidity for retail position sizing
5. Passes the geographic safety gate (US/Canada domicile)

---

## Inclusion Criteria Detail

### What Qualifies
- DoD AI/software command and control platforms
- Autonomous and unmanned systems manufacturers
- Hypersonic propulsion and thermal protection suppliers
- Space launch and strategic missile infrastructure
- Advanced materials and specialty alloys suppliers
- Defense electronics and harsh-environment subsystems
- IoMT sensors, tactical networks, and drone orchestration
- Additive manufacturing for defense/aerospace applications
- Precision aerospace component manufacturers

### What Disqualifies
- Prime contractors (LMT, RTX, NOC, GD, BA) unless 
  a documented next-gen technology unit qualifies as 
  a special case exception
- Pure software/cyber plays without defense hardware exposure
- International defense companies
- Consumer or commercial aerospace without defense exposure

### Removal Triggers
- Acquisition by a prime contractor
- Delisting or going private
- Revenue mix shifts away from defense below threshold
- Fundamental thesis violation (e.g. loss of key contracts)
- Liquidity falls below minimum threshold
- Federal investigation into foreign adversary control, 
  infiltration, or ownership — regardless of resolution 
  status
- Cash runway below 2 years for unprofitable companies

### Tier Definitions
- **Disruptor:** High-growth, software-first or pure-play 
  next-gen defense. R&D > 10% of revenue or revenue 
  growth > 15% YoY. Target weight: 15% per position.
- **Standard:** Established defense supply chain with 
  meaningful next-gen exposure. Target weight: 10% per position.
- **Anchor:** Diversified or prime-adjacent. Target weight: 
  5% per position.

---

## Active Universe

### AI & Autonomy

| Ticker | Company | Exchange | Description | Tier |
|--------|---------|----------|-------------|------|
| PLTR | Palantir Technologies | NASDAQ | Command software & decision layer for DoD. Unique — no public proxy for military AI command and control. Special case exception to picks-and-shovels rule. Premium valuation caps thesis score. | Disruptor |
| AVAV | AeroVironment | NASDAQ | Tactical loitering munitions & small UAS. Switchblade franchise and deep unmanned systems portfolio. | Disruptor |
| KTOS | Kratos Defense | NASDAQ | High-performance autonomous jet platforms & hypersonic target testbeds. Dual-domain: AI/Autonomy + Hypersonic. | Disruptor |

### Hypersonic, Space & Propulsion Infrastructure

| Ticker | Company | Exchange | Description | Tier |
|--------|---------|----------|-------------|------|
| KRMN | Karman Holdings | NYSE | Mission-critical payload protection & interstage structures for hypersonic and space launch vehicles. Purpose-built for this thesis. High debt/equity — monitor. | Disruptor |
| HWM | Howmet Aerospace | NYSE | Refractory alloys & extreme-temperature thermal protection. Builds ahead of demand for hypersonic programs. | Standard |
| ATI | ATI Inc | NYSE | Titanium alloys & structural high-speed airframes. Critical materials for hypersonic vehicles. Currently overbought — extended. | Standard |

### IoMT, Sensors & Digital Battlefield

| Ticker | Company | Exchange | Description | Tier |
|--------|---------|----------|-------------|------|
| AXON | Axon Enterprise | NASDAQ | Connected tactical sensors, software nodes, and drone orchestration. Dual-domain: IoMT + AI/Autonomy. Defense secondary to law enforcement today but rapidly expanding. | Disruptor |
| TDY | Teledyne Technologies | NYSE | Advanced defense electronics & harsh-environment imaging. Strong fundamentals, technically mixed. | Standard |
| HEI | Heico Corporation | NYSE | Proprietary subcomponents & defense electronic technologies. Best fundamental profile in universe. Technical signals historically unreliable — size conservatively. | Standard |
| LOAR | Loar Holdings | NYSE | High-margin precision aerospace subcomponents. Classic picks-and-shovels supply chain layer. | Standard |

### Industrial Supply Chain & Materials

| Ticker | Company | Exchange | Description | Tier |
|--------|---------|----------|-------------|------|
| MTRN | Materion | NYSE | Specialty beryllium alloys & optical/guidance subcomponents. Cross-market supplier — semi, auto, defense. | Anchor |
| VELO | Velo3D | NASDAQ | Advanced additive manufacturing for complex defense geometries. Pre-profitability, high risk, extreme volatility. | Anchor |

---

## Watchlist — Under Research

| Ticker | Company | Exchange | Domain | Status |
|--------|---------|----------|--------|--------|
| FLY | Firefly Aerospace | NASDAQ | Responsive space launch & tactical transit | IPO Aug 2025 — insufficient price history |
| AADX | Applied Aerospace & Defense | NYSE | Solid rocket motors & missile propulsion subsystems | IPO June 2026 — too new |
| BAH | Booz Allen Hamilton | NYSE | Defense services & AI deployment integration | Watchlist — consulting firm, weak picks-and-shovels fit |
| TDG  | TransDigm Group            | NYSE   | Highly engineered proprietary aerospace components | Strong picks-and-shovels fit — research defense revenue mix |
| CW   | Curtiss-Wright Corporation | NYSE   | Defense electronics, naval systems, motion control | Direct thesis fit — defense electronics and aerospace systems |
| HXL  | Hexcel Corporation         | NYSE   | Advanced carbon fiber composites for aerospace/defense | Composites in virtually every advanced defense platform |
---

## IPO Watch

These private companies are expected to go public and should 
be added to the universe immediately upon IPO:

| Company | Est. Valuation | Domain | Trigger |
|---------|---------------|--------|---------|
| Anduril Industries | ~$61B | AI/Autonomy, autonomous weapons | SEC S-1 filing |
| Shield AI | ~$12.7B | AI/Autonomy, Hivemind pilot software | SEC S-1 filing |

---

## Portfolio Rules

### Rebalancing
- Standard: Monthly
- Shock trigger: If any position exceeds 13.5% or 
  falls below 6.5% of total portfolio, force rebalance

### Position Sizing by Score
| Score | Signal | Action |
|-------|--------|--------|
| 75-100 | STRONG | Consider adding — up to tier maximum |
| 50-74 | NEUTRAL | Hold / monitor |
| 25-49 | WEAK | Consider trimming |
| 0-24 | CRITICAL | Review for removal |

### Valuation Risk Flags
- **PLTR:** High conviction thesis, elevated valuation 
  risk — size conservatively regardless of composite score
- **KRMN:** High debt/equity (213+) — monitor leverage
- **VELO:** Pre-profitability, extreme volatility — 
  watchlist only until stability improves

---

## Version History

| Version | Date | Notes |
|---------|------|-------|
| 0.1 | June 2026 | Initial universe draft |
| 0.2 | June 2026 | Added ATI, HEI, SXI, TDY. Added foreign adversary removal trigger. Deferred tier assignments |
| 0.3 | June 2026 | Major restructure. Added PLTR, AXON. Five-domain sector framework. Tier definitions. IPO watchlist. Portfolio rules. Cash runway removal trigger. Valuation risk flags |

